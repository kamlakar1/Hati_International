...
public class OPReg_Encounter_Insurance_ADT_OPtoIPconvert_SaveAdmission_GLApproval_IPcharge_Save 
{
	WebDriver driver;
	...
	{
		BillingPages_2.GLApproval_For_OP_IP_FOR_IP_Charge  GL=new BillingPages_2.GLApproval_For_OP_IP_FOR_IP_Charge(driver);
		GL.GLApprovalForOPToIP(PolicyNoPar,PolicyDatePar,HolderNamePar,CustomerNamePar,GLRefNOPar,GLDatePar,ExpiryDatePar,GLAmountPar,CoverageAmountPar,ContactNamePar,DesignationPar,ContactNoPar,GLRemarksPar);	 
	}
	@BeforeTest
	public void beforeTest() throws InterruptedException, ATUTestRecorderException 
	{
		recorder = new ATUTestRecorder(".//Videos//","OPReg_Encounter_Insurance_ADT_OPtoIPconvert_SaveAdmission_GLApproval_IPcharge_Save"+dateFormat.format(date),false);
		recorder.start();
	...
	public void FailedScreenShot(ITestResult Result) throws IOException
	{
		if(ITestResult.FAILURE==Result.getStatus())
		{
			...
			FileUtils.copyFile(screenshot1, new File(".//ScreenShotFailedTestCase//Failure_OPReg_Encounter_Insurance_ADT_OPtoIPconvert_SaveAdmission_GLApproval_IPcharge_Save"+ dateFormat.format(date)));
		}
	}
	@AfterSuite
